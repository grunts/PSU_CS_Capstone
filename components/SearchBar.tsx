...
const DropdownSearch = (props) => {
    ...
    const { colors, dark } = useTheme();
        return (
              ...
              inputStyle={!dark ? {color: 'black'} : {}}
               />
                ...
                contentContainerStyle={
                  filteredDataSource && search.length > 2 && filteredDataSource.length ? {marginBottom: (50*filteredDataSource.length)} : {}
                }
                />
            {
            ...
            {
              search.length > 2 && (filteredDataSource && filteredDataSource.length) ?
              <View style = {{height: 30, justifyContent: 'center', alignItems: 'center', backgroundColor: `${colors.background}`}}>
